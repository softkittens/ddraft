...
export type TurnEvaluation =
  ...
  | { action: "error"; reason: "thrashing" | "stalled"; message: string };
...
export class SessionWatchdog {
  ...
  evaluateCompletion(doc: Document, turn: number, maxTurns: number, docUnchanged: boolean): CompletionEvaluation {
    if (docUnchanged && doc.children.length > 0) {
      return { action: "finish" };
    }

    if (doc.children.length === 0 && this.corrections < MAX_CORRECTIONS && turn < maxTurns - 1) {
      this.corrections += 1;
      return {
        action: "retry_empty",
        nudge:
